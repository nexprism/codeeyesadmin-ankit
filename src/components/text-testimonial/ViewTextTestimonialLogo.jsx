...
import { useAddTextTestimonialMutation, useGetAllTextTestimonialQuery, useGetTextTestimonialQuery } from "../../redux/features/textTestimonialEndPoingt";
import TextArea from "antd/es/input/TextArea";
import { TextTestimonialValidation } from "../../commondata/formvalidations";

export default function ViewTextTestimonial() {
  const { id } = useParams()
  const navigate = useNavigate();
  const org = Cookies.get("organization")
  const { refetch, isLoading } = useGetAllTextTestimonialQuery(org);

  const { data, isLoading: loadingSIngle, isSuccess } = useGetTextTestimonialQuery(id)
  ...
  useEffect(() => {
    ...
    setFieldValue("image_url", singleData?.image_url)
  }, [isSuccess])

  return (
    <>
      {
        loadingAdd && <Loader /> || isLoading && <Loader /> || loadingSIngle && <Loader />
      }
      <Row>
        <Col>
          <PageHeader titles="Testimonial" active="View Text Testimonial" items={["Home"]} links={["/dashboard"]} />
        </Col>
      </Row>
      <Row>
        <Col>
                ...
                <Row className="mb-4">
                  <Col as={Col} md={6}>
                    <Form.Group as={Col}>
                      ...
                      <Form.Control disabled title="text" name="title" onChange={handleChange} onBlur={handleBlur} value={values.title} />
                  ...
                  <Col as={Col} md={6}>
                    <Form.Group as={Col}>
                      ...
                      <Form.Control disabled title="text" name="name" onChange={handleChange} onBlur={handleBlur} value={values.name} />
