...
import { useDeleteTextTestimonialMutation, useGetAllTextTestimonialQuery } from "../redux/features/textTestimonialEndPoingt";

export default function TextTestimonialsDataTable() {
  ...
  if (isSuccess) {
    const COLUMNS = [
      ...
      },
      {
        ...
        sortable: true,
        cell: (row) => {
          return <img src={row.image_url} alt={row.name} style={{ width: "100px", height: "auto" }} />;
        },
      },
          ...
          <>
            <div className="d-flex gap-1">
              <OverlayTrigger key="bottom" placement="bottom" overlay={<Tooltip id="tooltip-bottom">Edit</Tooltip>}>
                <Link to={`/view-text-textomonial/${row?.id}`}>
              ...
              <OverlayTrigger key="bottom" placement="bottom" overlay={<Tooltip id="tooltip-bottom">Edit</Tooltip>}>
                <Link to={`/edit-text-textomonial/${row?.id}`}>
    ...
    const filteredData = postsData?.filter((item) => {
      ...
      return item?.name?.toLowerCase().includes(searchTermLower) || item?.category?.toLowerCase().includes(searchTermLower);
    });
