...
import { useDeleteServicesMutation, useGetAllServicesQuery } from "../redux/features/servicesEndPoingt";
import { useDeleteEmailFooterMutation, useGetAllEmailFooterQuery } from "../redux/features/emailFooterEndPoint";

export default function EmailFooterDataTable() {
    ...
    if (isSuccess) {
                    ...
                    <>
                        <div className="d-flex gap-1">
                            <OverlayTrigger key="bottom" placement="bottom" overlay={<Tooltip id="tooltip-bottom">View</Tooltip>}>
                                <Link to={`/view-email-footer/${row?.id}`}>
                            ...
                            <OverlayTrigger key="bottom" placement="bottom" overlay={<Tooltip id="tooltip-bottom">Edit</Tooltip>}>
                                <Link to={`/edit-email-footer/${row?.id}`}>
        ...
        const filteredData = postsData?.filter((item) => {
            ...
            return item?.service?.toLowerCase().includes(searchTermLower) || item?.category?.toLowerCase().includes(searchTermLower);
        });
